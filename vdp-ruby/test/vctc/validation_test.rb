...
class ValidationAPITest < Test::Unit::TestCase
  ...
  def test_retrieveTransactionType
    ...
    url = "#{@config['visaUrl']}#{base_uri}#{resource_path}#{query_param}"
    ...
    puts " Retrieve List of Recent Decision Records test"
    puts url
    begin
      ...
      assert(false, "Retrieve List of Recent Decision Records test failed")
    end
  end
end